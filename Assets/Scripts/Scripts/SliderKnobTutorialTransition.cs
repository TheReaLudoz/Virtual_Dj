...
namespace KnobsAsset
{
    ...
    /// </summary>
    public class SliderKnobTutorialTransition : Knob
    {
        ...
        [Tooltip("The threshold below which the action is considered completed")]
        [SerializeField] private float CompletionThreshold = 0.1f;
        ...
        private void Update()
        {
            if (grabbed)
            {
                ...
                float dot = Vector3.Dot(transform.forward, mousePosOnAxis - handle.position);
                ...
                // Check if the completion threshold has been reached and complete the action
                if (AmountMoved == CompletionThreshold)
                {
                    SetVolumeActionCompleted();
                }

                // Check if the knob is at the maximum position and complete the action
                if (AmountMoved <= 1f)
                {
                    MaxVolumeActionCompleted();
                }
            }
        }
        ...
        private void SetVolumeActionCompleted()
        {
            FindObjectOfType<TutorialManager>().ActionCompleted(TutorialManager.TutorialAction.SetVolume);
        }

        private void MaxVolumeActionCompleted()
        {
            FindObjectOfType<TutorialManager>().ActionCompleted(TutorialManager.TutorialAction.MaxVolume);
        }
    }
}
