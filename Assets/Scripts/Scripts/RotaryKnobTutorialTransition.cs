...
namespace KnobsAsset
{
    ...
    /// </summary>
    public class RotaryKnobTutorialTransition : Knob
    {
        [System.Serializable]
        ...
        void Update()
        {
            if (grabbed)
            {
                ...
                // Check if the knob has reached the minimum angle
                if (AmountRotated <= -60f)
                {
                    // Call the ActionCompleted event of the TutorialManager with DecreaseEQValue action
                    FindObjectOfType<TutorialManager>().ActionCompleted(TutorialManager.TutorialAction.DecreaseEQValue);
                }
            }
        }
